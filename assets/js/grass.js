...
function wavingGrass() {
    ...
    img.src = 'https://i.imgur.com/zzjtzG7.jpg';
...
function animMeter(q, a) {
    ...
    this.update = function(y) {
        var h, w, i;
        if (p || (arguments.length === 1 && typeof y !== "undefined")) {
            if (u === null) {
                u = 0;
                p = true;
                return
            ...
            if (u === null) { u = (new Date()).getTime(); return }
            j = (new Date()).getTime();
            i = j - u;
            u = j
        }
        ...
        r.lineTo(c, x);
        r.stroke();
        r.fillRect(c, k, n - c, b);
        if (g === 0) {
            r.beginPath();
            r.fillRect(n, 0, z, t);
            r.fillStyle = "#ddf";
            r.fillText(e, n + z + o - 2, -1 + m);
            r.fillStyle = "#000";
            g++;
            e = w
        } else {
    ...
    d.width = f;
    d.height = t;
    d.title = "animMeter v.1.0 by Ken Fyrstenberg http://abdiassoftware.com/";
    ...
    for (; s < 4; s++) {
        r.beginPath();
        r.strokeStyle = l[s];
        r.moveTo((s * n * 0.25) | 0, k - 1);
        r.lineTo((s * n * 0.25 + n * 0.25 - ((s < 3) ? 1 : 0)) | 0, k - 1);
        r.stroke()
    }
